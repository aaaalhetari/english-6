...
const { seedDemo, getStats, getAllWords } = useVocabDB()
    ...
    <div class="max-w-3xl mx-auto px-4 py-6">
      ...
      <div class="flex items-center justify-between mb-5">
        ...
        <div class="text-[11px] text-slate-500 flex gap-3">
          <span>active {{ stats.active }}</span><span>dormant {{ stats.inactive }}</span><span>pool {{ stats.pool }}</span>
        </div>
      </div>
      ...
      <div class="mt-4 text-xs text-slate-400 space-y-1">
        <p>• <strong>One tap</strong> on a highlighted word says it out loud. <strong>Two taps</strong> open its meaning card.</p>
